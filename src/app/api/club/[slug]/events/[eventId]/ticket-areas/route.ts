...
import { createTicketAreaSchema } from "@/lib/shared/schemas/clubDashboard";
...
export async function POST(
  ...
  const parsed = createTicketAreaSchema.safeParse(body);
  if (!parsed.success) {
  ...
  const area = await prisma.ticketArea.create({
    data: {
      eventId,
